...
def get_input_data(logfile, functional, a1, b1, print_level=1):
    ...
    return {
        ...
        "nuclear_charge": float(
            get_info(contents, charge) + get_info(contents, nelecs)
        ),
    ...
    for i in range(nocc.size):
        ...
        for j in range(i):
            if i < homo and j < homo:
                ...
                els[i, j] = (
                    -0.5 * np.sqrt(nocc[i] * nocc[j]) * twoeno[ijkl(i, j, i, j)]
                    + 0.5 * nocc[i] * nocc[j] * twoeno[ijkl(i, i, j, j)]
                )
            els[j, i] = els[i, j]
    return els
        ...
        for j in range(i):
            if i < homo and j < homo:
                ...
                els[i, j] = (
                    -0.5 * np.sqrt(nocc[i] * nocc[j]) * twoeno[ijkl(i, j, i, j)]
                    + 0.5
                    * (np.sqrt(nocc[i] * nocc[j]) - 0.5 * nocc[i] * nocc[j])
                    * twoeno[ijkl(i, j, i, j)]
                    * pade_ac3(a, b, 0.5 * nocc[i] - 0.5)
                    + 0.5 * nocc[i] * nocc[j] * twoeno[ijkl(i, i, j, j)]
                )
            els[j, i] = els[i, j]
    return els
