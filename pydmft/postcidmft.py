...
def print_energies(fun_name, energies, components=False):
    ...
    if components:
        for row in sorted(energies, key=itemgetter("dist")):
            print "{0:<30s}  {1:6.2f}{2:20.10f}{3:20.10f}{4:20.10f}{5:20.10f}{6:20.10f}".format(row["file"],
                row["dist"], row["jcomps"]["inner diagonal"],
    ...
    for (path, dirs, files) in os.walk(cases[args['<molecule>']]['workdir']):
        for fileitem in files:
            if "_NO.log" in fileitem:
                ...
                exact.jk_rdms(occ.size, rdm2=rdm2, Jints=Jints, Kints=Kints, homo=gp.get_homo())
...
def main():
    ...
    if not args['<molecule>'] in cases.keys():
        sys.exit('wrong molecule: {0:s}, should be one of (lih, behp, li2)'.format(args['<molecule>']))


    functionals = [ELS2('ELS2'), PNOF4("PNOF4"), ELS1mod("ELS1mod"), ELS1("ELS1"), DD("DD")]
